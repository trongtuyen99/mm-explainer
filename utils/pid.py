...
def clustering(X, n_clusters=20):
    print(f"Clustering with shape: {X.shape}")
    X = np.nan_to_num(X)
    if len(X.shape) > 2:
...
def get_measure_from_data(X1, X2, y, num_cluster):
    '''
    X1: Input matrix
    X2: Input matrix
    y: Input labels
    ...
    '''
    X1 = clustering(X1, n_clusters=num_cluster)
    X2 = clustering(X2, n_clusters=num_cluster)
    P, maps = convert_data_to_distribution(X1, X2, y)
    result = get_measure(P)
    return result


def agreement(truth_measure, pred_measure):
    sum_truth = sum(truth_measure.values())
    agree = copy.deepcopy(truth_measure)
    for i in agree:
        agree[i] = (truth_measure[i] / sum_truth) * pred_measure[i]

    return agree


if __name__ == "__main__":
    X1 = np.random.rand(300, 50);  # (num_data, dim)
    X2 = np.random.rand(300, 100);  # (num_data, dim)
    y_true = np.random.randint(6, size=(300))  # 6 class
    y_pred = np.random.randint(6, size=(300))  # 6 class

    truth_measure = get_measure_from_data(X1, X2, y_true, num_cluster=6)
    print(truth_measure)
    pred_measure = get_measure_from_data(X1, X2, y_pred, num_cluster=6)
    print(pred_measure)
    agree = agreement(truth_measure, pred_measure)
    print(agree)