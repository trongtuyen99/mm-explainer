...
class MULTModel(nn.Module):
    ...
    def __init__(self, n_modalities, n_features, hyp_params=DefaultHyperParams, use_modal=None, sample=True):
    ...
    def forward(self, x):
        ...
        """
        if len(self.use_modal) < 3:
            x = [x[i] for i in self.use_modal]
        x = [v.permute(0, 2, 1)
        ...
        # print(f'Input shape after projection: {[X.shape for X in proj_x]}')
        # return proj_x
