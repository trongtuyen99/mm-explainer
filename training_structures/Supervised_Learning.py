...
def single_test(
        ...
        input_to_float=True, output_folder='.'):
    ...
    with torch.no_grad():
        ...
        if task == "classification":
            ...
            write_pickle(pred, f'{output_folder}/prediction.pickle')
            write_pickle(trueposneg, f'{output_folder}/truth.pickle')
...
def test(
        ...
        auprc=False, input_to_float=True, no_robust=False, output_folder='.'):
    """
    ...
    if no_robust:
        def _testprocess():
            single_test(model, test_dataloaders_all, is_packed,
                        criterion, task, auprc, input_to_float, output_folder=output_folder)
        all_in_one_test(_testprocess, [model])
        return

    def _testprocess():
        single_test(model, test_dataloaders_all[list(test_dataloaders_all.keys())[
                    0]][0], is_packed, criterion, task, auprc, input_to_float, output_folder=output_folder)
    all_in_one_test(_testprocess, [model])
