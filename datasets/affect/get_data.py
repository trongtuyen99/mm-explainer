...
class Affectdataset(Dataset):
    ...
    def __init__(self, data: Dict, flatten_time_series: bool, aligned: bool = True, task: str = None, max_pad=False,
                 max_pad_num=50, data_type='mosi', z_norm=False) -> None:
...
def get_dataloader(
    ...
    train = DataLoader(Affectdataset(processed_dataset['train'], flatten_time_series, task=task, max_pad=max_pad,
                       ...
                       collate_fn=process)
    valid = DataLoader(Affectdataset(processed_dataset['valid'], flatten_time_series, task=task, max_pad=max_pad,
                                     max_pad_num=max_seq_len, data_type=data_type, z_norm=z_norm), \
    ...
    if robust_test:
        ...
        for i in range(10):
            ...
            robust_text.append(
                DataLoader(Affectdataset(test, flatten_time_series, task=task, max_pad=max_pad, max_pad_num=max_seq_len,
                                         data_type=data_type, z_norm=z_norm), shuffle=False, num_workers=num_workers,
        ...
        for i in range(10):
            test = dict()
            test['vision'] = \
                add_timeseries_noise([alldata['test']['vision'].copy()], noise_level=i / 10, rand_drop=False)[0]
        ...
        for i in range(10):
            ...
            test['audio'] = \
                add_timeseries_noise([alldata['test']['audio'].copy()], noise_level=i / 10, rand_drop=False)[0]
            test['text'] = alldata['test']['text'].copy()
            test['labels'] = alldata['test']["labels"]
            test = drop_entry(test)
            print('test entries: {}'.format(test['vision'].shape))
        ...
        # test = dict()
        test = DataLoader(Affectdataset(processed_dataset['test'], flatten_time_series, task=task, max_pad=max_pad,
                                        max_pad_num=max_seq_len, data_type=data_type, z_norm=z_norm), \
                          shuffle=False, num_workers=num_workers, batch_size=batch_size, \
                          collate_fn=process)
...
if __name__ == '__main__':
    traindata, validdata, test_robust = \
        get_dataloader('./humor.pkl', robust_test=False, max_pad=True, task='classification', data_type='humor',
                       max_seq_len=40)
